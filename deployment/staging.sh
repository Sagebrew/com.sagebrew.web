...
#  --version-label $SHA1 --template-name staging-web-conf --tier Name=WebServer,Type=Standard,Version=1.0 --option-settings file://$DOCKERRUN_WEB_ENVIRONMENT
...
aws elasticbeanstalk update-environment --option-settings file://$DOCKERRUN_WORKER_ENVIRONMENT --environment-name sb-staging-worker \
    --version-label $SHA1