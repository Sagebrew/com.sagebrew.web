...
export function load() {
    var questionID = helpers.args(2),
        ...
        questionForm = document.getElementById('questionInputForm'),
        questionValidationForm = $(questionForm),
        editor = mediumEditor(".editable", "Type your Question context here");
    validators.questionValidator(questionValidationForm);
    locations.init();
    $secondnav.on('click', '#submit', function(event) {
        ...
        if (settings.profile.privileges.indexOf("comment") > -1 || regexMatches) {
            ...
            request.patch({url: "/v1/questions/" + questionID + "/", data: data})
                .done(function (data) {
