...
class Command(BaseCommand):
    ...
    def populate_universities(self):
        '''
        This function accesses the local file university_list.csv, converts it into a
        dictionary then creates a node of Universiy with the information in each dictionary
    ...
    def populate_public_schools(self):
        '''
        This function accesses the local file public_school_list.csv, converts each row
        into a dictionary then populates a HighSchool node only if the name of the school
        contains 'HIGH'.
        ...
        for row in public_schools_dict:
            row.pop('', None)
            if 'HIGH' in row['school_name']:#TODO Add better filter for more accurate results
                if row['phone_number']=='M' or 'N':#TODO Add better way to handle invalid phone_number
                    row['phone_number'] = 0
